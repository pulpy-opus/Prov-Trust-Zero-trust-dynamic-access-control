...
# Time encoding module for TGAT
class TimeEncode(torch.nn.Module):
    def __init__(self, out_channels):
        super(TimeEncode, self).__init__()
        self.out_channels = out_channels
        self.w = torch.nn.Parameter(torch.from_numpy(
            np.random.uniform(0, 1, (out_channels,)).astype(np.float32)))
    def forward(self, t):
        # t: [N] or [E], float tensor, unit: nanosecond timestamp
        t = t.unsqueeze(-1)  # [N, 1] or [E, 1]
        # Normalize timestamp to avoid numerical overflow
        t = t / 1e9
        output = torch.cos(t * self.w)
        return output
...
class GraphAttentionEmbedding(torch.nn.Module):
    def __init__(self, in_channels, out_channels, msg_dim, time_enc):
        ...
        # Input projection layer: project msg_dim up to in_channels
        self.input_proj = Linear(msg_dim, in_channels)
    ...
    def forward(self, x, last_update, edge_index, t, msg):
        ...
        x = x.to(device)
        t = t.to(device)
        # Project input features to in_channels dimension
        x = self.input_proj(x)
