...
def test(inference_data,
    ...
    for batch in loader:
        ...
        x = inference_data.msg[n_id, :node_state_dim]
        last_update = inference_data.t[n_id]
        z = gnn(x, last_update, edge_index, inference_data.t[e_id], inference_data.msg[e_id])
        ...
        for i in range(len(pos_out)):
            ...
            loss_val = each_edge_loss[i]
            temp_dic = {}
            temp_dic['loss'] = float(loss_val)
...
if __name__ == "__main__":
    ...
    # load trained model
    gnn, link_pred, neighbor_loader = torch.load(f"{models_dir}/models.pt",map_location=device)
